 ...
 */
export function QuizSession({ client, onStart }: Props) {
  const [all, setAll] = useState<QuizSummary[]>([]);
  ...
  useEffect(() => {
    let active = true;
    void (async () => {
      const list = await client.searchQuizzes({});
      if (active) {
        setAll(list);
        setLoading(false);
      }
  ...
  // 全クイズに出現するタグ（絞り込みチップ用）
  const allTags = useMemo(() => [...new Set(all.flatMap((q) => q.tags))].sort(), [all]);

  // 条件に一致するクイズ
  const filtered = useMemo(
    () =>
      all.filter((q) => {
        if (favoriteOnly && !q.favorite) return false;
        if (q.questionCount < minQuestions) return false;
  ...
  // 一致 かつ 個別選択済み = 実際の受験対象
  const target = useMemo(() => filtered.filter((q) => selected.has(q.id)), [filtered, selected]);
  const targetQuestionTotal = target.reduce((sum, q) => sum + q.questionCount, 0);
